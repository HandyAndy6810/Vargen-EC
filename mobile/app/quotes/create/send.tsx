 ...
 *
 * `shareAnyway` is handed in by Review rather than rebuilt here, because generating
 * the PDF needs the same payload the preview uses.
 */
export default function SendGateSheet() {
  ...
  /**
   * Writes a real customer record so the quote is linked to someone who exists
  ...
  const createAndSend = async () => {
    ...
    try {
      ...
      // Even if the record couldn't be created, keep the name so the quote can still
  ...
  return (
    <View style={s.wrap}>
      ...
      >
        ...
        <Text style={s.sub}>A quote needs a customer before it can be sent.</Text>
